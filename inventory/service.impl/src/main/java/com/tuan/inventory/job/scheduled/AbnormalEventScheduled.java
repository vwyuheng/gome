...
//						System.out.println("execFixedRate4Abnormal2");
	...
	class AbnormalQueueConsumeTask implements Runnable {
		...
		public void run() {
			...
			List<GoodsInventoryQueueModel> queueList = null;
			try {
				//取初始状态队列信息
				queueList = goodsInventoryDomainRepository
							...
							if(statEnum!=null)
							{
								try {
									...
									if (statEnum
										...
										result = goodsInventoryDomainRepository
												.queryGoodsInventoryByGoodsId(model
																.getGoodsId());
									...
									goodsInventoryDomainRepository.markQueueStatus(
