...
public class ShardedJedisSentinelPool extends Pool<ShardedJedis> {
    ...
    public ShardedJedisSentinelPool(List<String> masters, Set<String> sentinels,
		...
		//initPool(masterList);
		List<HostAndPort> isslaveList = null;  //存需用到的
		List<HostAndPort> slaveList = initSentinelSlaves(sentinels, masters);
		if(!CollectionUtils.isEmpty(slaveList)) {
			isslaveList = new ArrayList<HostAndPort>();
			isslaveList.add(slaveList.get(Utils.random(slaveList.size())));
			initPool(isslaveList);
		}
    ...
    private void initPool(List<HostAndPort> masters) {
    	if (!equals(currentHostMasters, masters)) {
    		...
    		log.info("Created ShardedJedisPool to slave at [" + sb.toString() + "]");
	...
	private List<HostAndPort> initSentinelSlaves(Set<String> sentinels, final List<String> masters) {
		
		Map<String, HostAndPort> masterMap = new HashMap<String, HostAndPort>();
		//Map<String, HostAndPort> slavesMap = new HashMap<String, HostAndPort>();
		List<HostAndPort> shardMasters = new ArrayList<HostAndPort>();
		List<HostAndPort> shardSlaves = new ArrayList<HostAndPort>();
							...
							if (hostAndPort != null && hostAndPort.size() > 0) {
								...
								if (slaves != null && slaves.size() > 0) {
									for (Map<String, String> map : slaves) {
										...
										shardSlaves
												.add(toHostAndPort(hostAndPortSalve));
									}
								}
								fetched = true;
								jedis.disconnect();
